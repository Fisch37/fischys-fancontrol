...
use crate::{
    GlobalContext,
    utils::{ErrorGroup, ExitStatusError, SimpleError},
};
...
pub fn query_sensors(
    ...
    for result in reader.records() {
        match result {
            ...
            Ok(record) => {
                if record.len() < 5 {
                    error.push(SimpleError::new(format!(
                        "Unexpected record length. Expected 5, found {}",
                        record.len()
                    )));
                    continue;
                }
                ...
                match record[2].trim().parse() {
                    ...
                    Ok(core_temperature) => {
                        match temperature_sensors.add(SensorData {
                            ...
                            Ok(_) => {}
                            Err(_) => warn!(
                                "Tried to add duplicate sensor core_temperature for {:?}",
                                adapter
                            ),
                        }
                    }
                ...
                match record[4].trim().parse() {
                    ...
                    Ok(memory_temperature) => {
                        match temperature_sensors.add(SensorData {
                            ...
                            Ok(_) => {}
                            Err(_) => warn!(
                                "Tried to add duplicate sensor memory_temperature for {:?}",
                                adapter
                            ),
                        }
                    }
