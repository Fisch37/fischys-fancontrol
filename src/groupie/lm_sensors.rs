...
use crate::{
    GlobalContext,
    utils::{ExitStatusError, MalformedDataError},
};

use super::{Adapter, QueryResult, SensorData, SensorKind};
...
pub fn query_sensors(
    ...
    #[allow(unused)] context: &GlobalContext,
) -> Result<(), Box<dyn Error>> {
    let result = Command::new("sensors").arg("-j").output()?;
    ...
    let root: serde_json::Map<String, serde_json::Value> =
        serde_json::from_slice(result.stdout.as_slice())?;
    for (adapter_key, adapter_value) in root {
        ...
        let adapter_raw = match adapter_value.as_object() {
            None => {
                return Err(Box::new(MalformedDataError::new_with_string(format!(
                    "Expected object for adapter, not {}",
                    adapter_value
                ))));
            }
            Some(x) => x,
        ...
        let adapter = Rc::new(Adapter {
            ...
            name: match adapter_raw.get("Adapter").and_then(|val| val.as_str()) {
                None => {
                    return Err(Box::new(MalformedDataError::new(
                        "Key value Adapter for Adapter not found or of incorrect type",
                    )));
                }
                Some(x) => x.to_string(),
            },
        ...
        for (sensor_key, sensor_value) in adapter_raw {
                ...
                match key_parts.next() {
                    ...
                    None => {
                        return malformed_data_error(
                            "Sensor key does not contain an _ (should be <key>_<dtype>)",
                        );
                    }
                }
            }
            match kind {
                None => warn!(
                    "Skipping unknown sensor of unknown kind {}/{}",
                    sensor.adapter.key, sensor.name
                ),
                Some(k) => {
                    sensor.kind = k;
                    state.add(sensor).map_err(|sensor| {
                        Box::new(MalformedDataError::new_with_string(format!(
                            "Duplicate sensor {}/{}",
                            sensor.adapter.key, sensor.name
                        )))
                    })?
