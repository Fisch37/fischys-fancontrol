...
use crate::{
    ...
    groupie::{SensorKey, SensorStorage},
    utils::SimpleError,
};
...
const fn default_curve_mode() -> CurveMode {
    CurveMode::LinearInterpolation
}
    ...
    U: FnMut((&str, f64, f64)),
{
    ...
    for pwm in pwms.iter_mut().map(AsMut::as_mut) {
        ...
        let combined_temp = match curve.input_sensors.evaluate(state) {
            None => {
                warn!(
                    "Failed to get sensor data for pwm {}. Sensor not found",
                    pwm_name
                );
                continue;
            }
        ...
        let low_index: Option<usize> = curve
            .points
            .iter()
            .enumerate()
        ...
        let pwm_value: f64 = f64::clamp(
            match low_index {
                None => curve.min,
                Some(low_index) => match high_index {
                    None => curve.max,
                    Some(high_index) => {
                            ...
                            None => {
                                push_err(
                                    ...
                                    SimpleError::new(format!(
                                        "Out of bounds low index {low_index} for {pwm_name}"
                                    )),
                                );
                                continue;
                            }
                        ...
                        let high = match curve.points.get(high_index) {
                            ...
                            None => {
                                push_err(
                                    &mut errors,
                                    SimpleError::new(format!(
                                        "Out of bounds high index {high_index} for {pwm_name}"
                                    )),
                                );
                                continue;
                            }
        ...
        if log_enabled!(log::Level::Info) {
            info!(
                "{}: {:.1}°C (li {:?}; hi {:?}) -> {}",
                pwm_name, combined_temp, low_index, high_index, pwm_value
            );
        }
